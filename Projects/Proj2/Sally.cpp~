   ...
   istrm(input_stream)  // use member initializer to bind reference
{
   ...
   symtab["!"]  =  SymTabEntry(KEYWORD,0,&doECL);

   symtab["<"]  =  SymTabEntry(KEYWORD,0,&doLessTh);
   symtab["<="]  =  SymTabEntry(KEYWORD,0,&doLessThE);
   symtab["=="]  =  SymTabEntry(KEYWORD,0,&doEE);
   symtab["!="]  =  SymTabEntry(KEYWORD,0,&doNotE);
   symtab[">"]  =  SymTabEntry(KEYWORD,0,&doGreatTh);
   symtab[">="]  =  SymTabEntry(KEYWORD,0,&doGreatThE);
...
void Sally::doDup(Sally *Sptr){
  ...
  p = Sptr -> params.top(); //creates a temp copy of the top token
  Sptr -> params.push( Token(p.m_kind, p.m_value, "") ) ; //adds the copy to the stack
}

void Sally::doDrop(Sally *Sptr){
  if ( Sptr -> params.size() < 1 ){
    throw out_of_range("Need on parameter for DROP");
  }
  Sptr -> params.pop(); //simply remove the top of the stack
}
...
void Sally::doSwap(Sally *Sptr){
  ...
  p1 = Sptr -> params.top(); //p1 = top value
  Sptr -> params.pop();
  p2 = Sptr -> params.top(); //p2 = second from top value 
  Sptr -> params.pop();

  Sptr -> params.push( Token(p1.m_kind, p1.m_value, ""));
  Sptr -> params.push( Token(p2.m_kind, p2.m_value, ""));
}
...
void Sally::doRot(Sally *Sptr){
  ...
  Sptr -> params.push( Token(p2.m_kind, p2.m_value, ""));
  Sptr -> params.push( Token(p1.m_kind, p1.m_value, ""));
  Sptr -> params.push( Token(p3.m_kind, p3.m_value, ""));
}
...
void Sally::doSET(Sally *Sptr){
  ...
  p1 = Sptr -> params.top(); // p1 = Variable name
  Sptr -> params.pop();
  //CHECK LATER TO SEE IF IT IS AN INTEGER
  p2 = Sptr -> params.top(); // p2 = Value to store
  Sptr -> params.pop();

  map<string,SymTabEntry>::iterator it = Sptr -> symtab.find(p1.m_text);
...
void Sally::doECL(Sally *Sptr){
  ...
  p1 = Sptr -> params.top(); // p1 = variable name
  Sptr -> params.pop();
  p2 = Sptr -> params.top(); // p2 = new value
  Sptr -> params.pop();
...
void Sally::doLessTh(Sally *Sptr){
  Token p1, p2;
  if(Sptr -> params.size() < 2)
    throw out_of_range("Need two parameters for <");

  p1 = Sptr -> params.top(); //rhs
  Sptr -> params.pop();
  p2 = Sptr -> params.top(); //lhs
  Sptr -> params.pop();
  int answer;
...
void Sally::doLessThE(Sally *Sptr){
  Token p1, p2;
  if(Sptr -> params.size() < 2)
    throw out_of_range("Need two parameters for <");
...
void Sally::doGreatTh(Sally *Sptr){
  Token p1, p2;
  if(Sptr -> params.size() < 2)
    throw out_of_range("Need two parameters for <");
...
void Sally::doGreatThE(Sally *Sptr){
  Token p1, p2;
  if(Sptr -> params.size() < 2)
    throw out_of_range("Need two parameters for <");
...
void Sally::doAND(Sally *Sptr){
  ...
  p1 = Sptr -> params.top(); // p1 is rhs
  Sptr -> params.pop();
  p2 = Sptr -> params.top(); // p2 is lhs
  Sptr -> params.pop();
  int answer;
  if(p2.m_value && p1.m_value)
    answer = 1;
...
void Sally::doIFTHEN(Sally *Sptr){
  ...
  if(p1.m_value != 0){
    ...
    while(p2.m_text != "ELSE" || numIF != numEL){
      p2 = Sptr -> nextToken();
      if(p2.m_text == "IFTHEN")
	numIF++;
      if(p2.m_text == "ELSE"){
...
void Sally::doELSE(Sally *Sptr){
  ...
  if(p1.m_value == 0){
    ...
    p2 = Sptr -> nextToken();
    while(p2.m_text != "ENDIF" || numENDIF != numEL){
      p2 = Sptr -> nextToken();
      if(p2.m_text == "ELSE")
	numEL++;
      if(p2.m_text == "ENDIF"){
...
void Sally::doDo(Sally *Sptr){
  if(tkList.size() > 0){
    while(tkList.size() > 0){ //if the tkList already has values, reset it
      tkList.pop_back();
    }
  }
  Token p1;
  while(p1.m_text != "UNTIL"){
    //cout << "Adding: " << p1.m_text << " to tkList" << endl;
    p1 = Sptr -> nextToken();
    tkList.push_back(p1);
  }
  list<Token> tempList = tkList;
  Sptr -> tkBuffer.splice(Sptr -> tkBuffer.begin(), tempList);
}
...
void Sally::doUntil(Sally *Sptr){
  ...
  Token p1;
  p1 = Sptr -> params.top();
  //  cout << "Condition is: " << p1.m_value << endl;
  if(p1.m_value == 0){
    Sptr -> tkBuffer.splice(Sptr -> tkBuffer.begin(), tempList);
  }
  Sptr -> params.pop();
